  ...
  const cards = [
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" className="w-24 h-24 text-green-500">
          <path fill="currentColor" d="M30,20 L30,80 M30,80 L90,80 M60,20 L60,50 M60,20 L90,20 M90,20 L90,50 M60,50 L90,50 M30,20 L60,20" 
            stroke="currentColor" strokeWidth="4" />
        </svg>
      ),
      description: "Voluptate velit cillum dol fugiat pariatur. Excepteur anim idet laboru sed perspise tis undy omnis iste natus."
    },
    {
  ...
  return (
    <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8 p-10">
      ...
      <div 
        ...
        style={{
          backgroundImage: "url('https://azim.commonsupport.com/Amortez/assets/images/shape/shape-6.png')"
        }}
      />
      ...
      {cards.map((card, index) => (
        <div 
          key={index} 
          className="bg-white p-10  shadow-lg relative overflow-hidden z-10 transition-all duration-300 transform hover:-translate-y-3 group"
        >
          <div className="mb-8 text-green-500">
            {card.icon}
          </div>
          <h2 className="text-4xl font-bold mb-8">{card.title}</h2>
          <hr className="w-4/5 border-gray-300 mb-8" />
          <p className="text-gray-600 mb-12 text-xl">{card.description}</p>
